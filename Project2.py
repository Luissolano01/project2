...
def get_titles_from_search_results():
    ...
    title = soup.find_all("td", {"class": "bookTitle"})

    author = soup.find_all("div", {"class": "authorName__container"})

    rating = soup.find_all("span", {"class": "greyText smallText uitext"})


    title = title.strip()
    author = author.stript()
    rating = rating.strip()

    group = title, author, rating 
...
def get_search_links():
    ...
    soup = BeautifulSoup(resp.content, 'html.parser')
    link_info = soup.find_all("a", {"class": "bookTitle"})
    link_info = link_info.strip()
